...
describe('RecipeInProgress drinkPath', () => {
  ...
  beforeEach(async () => {
    ...
    await act(async () => {
      const component = renderWithRouter(
        <DataProvider>
          <RecipeInProgress location={ { pathname: drinkPath } } />
        </DataProvider>,
        [drinkPath],
      );
  ...
  test('Se os elementos estão na tela', () => {
    const title = screen.getByText(/margarita/i);
    const image = screen.getByTestId(/recipe-photo/i);
    const instructions = screen.getByText(/rub the rim/i);
